...
CaptureNode::CaptureNode(std::shared_ptr<ros::NodeHandle> nodeHandle, const dv_ros_node::Params &params) :
	...
	if (mParams.triggers) {
		...
		mTriggerPublisher = mNodeHandle->advertise<std_msgs::Time>("triggers", 10);
	}
	...
	if (cameraPtr != nullptr) {
		if (cameraPtr->isFrameStreamAvailable()) {
			...
			// create a eventsRPFiler
            if (mParams.eventsRPFiltering) {
                mEventsRPFilter = std::make_unique<dv::RefractoryPeriodFilter<>>(
                    mReader.getEventResolution().value(), dv::Duration(mParams.eventsRPTime));
                    ROS_INFO_STREAM("Events refractory period filter enabled with time: " << mParams.eventsRPTime);
            }else{
                mEventsRPFilter = nullptr;
            }
			...
			while (triggerData.has_value() && !triggerData->empty() && timestamp >= triggerData->back().timestamp) {
				if (mTriggerPublisher.getNumSubscribers() > 0) {
					for (const auto &trigger : *triggerData) {
						...
						TriggerMessage message = dv_ros_msgs::toRosTriggerMessage(trigger);
						std_msgs::Time msg;
        				msg.data = message.timestamp;
						mTriggerPublisher.publish(msg);
						// ros::Time time_now = ros::Time::now();
						// std::cout << "now: " << std::to_string(time_now.toSec()) << std::endl; 
						// std::cout << "msg: " << std::to_string(msg.data.toSec()) << std::endl; 
						// std::cout << "dif: " << std::to_string(time_now.toSec() - msg.data.toSec()) << std::endl;
					}
				}
	...
	while (mSpinThread) {
		mEventsQueue.consume_all([&](const int64_t timestamp) {
			...
			while (events.has_value() && !events->isEmpty() && timestamp >= events->getHighestTime()) {
                ...
                // Evnets filtering by mNoiseFilter
                if (mNoiseFilter != nullptr) {
                    mNoiseFilter->accept(store);
                    store = mNoiseFilter->generateEvents();
                }

                // Evnets filtering by mEventsRPFilter
                if (mEventsRPFilter != nullptr) {
                    mEventsRPFilter->accept(store);
                    store = mEventsRPFilter->generateEvents();
                }
