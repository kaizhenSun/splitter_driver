...
ParametersLoader::ParametersLoader(const ros::NodeHandle &nodeHandle) {
	...
	nodeHandle.param<bool>("eventsRPFiltering", params_.eventsRPFiltering, false);
    nodeHandle.param<int>("eventsRPTime", value, 1000);
    params_.eventsRPTime = static_cast<int64_t>(value);
...
void ParametersLoader::printConfiguration() {
    ...
    ROS_INFO_STREAM("refractoryPeriodFiltering: " << (params_.eventsRPFiltering ? "yes" : "no"));
    ROS_INFO_STREAM("refractoryPeriodTime: " << params_.eventsRPTime);
    ROS_INFO_STREAM("--------- End of Filtering parameters ---------");
